variable "bucket_name_production" {
  type    = string
  default = "dsfds-production"
}
variable "bucket_name_staging" {
  type    = string
  default = "dsfds-staging"
}
  ...
  "Statement": [
    {
      ...
      "Principal": "*",
      "Action": "s3:GetObject",
      "Resource": "arn:aws:s3:::dsfds-production/*"
    }
  ]
}
EOF
  ...
  "Statement": [
    {
      ...
      "Principal": "*",
      "Action": "s3:GetObject",
      "Resource": "arn:aws:s3:::dsfds-staging/*"
    }
  ]
}
EOF
...
resource "aws_cloudfront_distribution" "s3_distribution_production" {
  ...
  comment             = "Production dsfds CloudFront"
...
resource "aws_cloudfront_distribution" "s3_distribution_staging" {
  ...
  http_version        = "http2"
  is_ipv6_enabled     = true
  comment             = "Staging dsfds CloudFront"
  default_root_object = "index.html"
