variable "bucket_name_production" {
  type    = string
  default = "tf-test-deploy-production"
}
variable "bucket_name_staging" {
  type    = string
  default = "tf-test-deploy-staging"
}
  ...
  "Statement": [
    {
      ...
      "Principal": "*",
      "Action": "s3:GetObject",
      "Resource": "arn:aws:s3:::tf-test-deploy-production/*"
    }
  ]
}
EOF
  ...
  "Statement": [
    {
      ...
      "Principal": "*",
      "Action": "s3:GetObject",
      "Resource": "arn:aws:s3:::tf-test-deploy-staging/*"
    }
  ]
}
EOF
...
resource "aws_cloudfront_distribution" "s3_distribution_production" {
  ...
  comment             = "Production tf-test-deploy CloudFront"
...
resource "aws_cloudfront_distribution" "s3_distribution_staging" {
  ...
  http_version        = "http2"
  is_ipv6_enabled     = true
  comment             = "Staging tf-test-deploy CloudFront"
  default_root_object = "index.html"
